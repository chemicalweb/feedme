...
def setup_routing(app, routes):
    ...
    a tuple where the first element is a '(blueprint, url_prefix)' tuple (or
    None if routes don't belong to a blueprint) and all the other elements
    are '(route, view_func)' tuples, add all the routes and register the
    blueprint to which they belong.
    """
    for route in routes:
